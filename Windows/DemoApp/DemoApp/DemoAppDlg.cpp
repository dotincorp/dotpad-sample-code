...
BOOL CDemoAppDlg::OnInitDialog() {
	...
	dot_pad_register_callback(DisplayDialogBoxByKeyNoti);
...
void CDemoAppDlg::LoadDotPadDLL() {
	...
	dot_pad_register_callback = (DOT_PAD_REGISTER_CALLBACK_FUNC)GetProcAddress(m_hDotPadDLL, "DOT_PAD_REGISTER_CALLBACK");
	...
	if (dot_pad_register_callback == NULL) {
		AfxMessageBox(_T("Can not find funtion DOT_PAD_REGISTER_CALLBACK_FUNC in DotPadSDK.dll!"));
	}
}
...
void CDemoAppDlg::FreeDotPadDLL() {
	...
	dot_pad_register_callback = NULL;
}
...
void CDemoAppDlg::OnBnClickedButtonUpData() {
	...
	DOT_PAD_SDK_ERROR error = dot_pad_display_data(data, 300);
